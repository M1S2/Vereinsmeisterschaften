...
public partial class CreateDocumentsViewModel : ObservableObject, INavigationAware
{
    ...
    private void initPlaceholderViewConfigs()
    {
        ...
        foreach(KeyValuePair<string, (string, List<string>)> placeholders in Placeholders.PlaceholderDict)
        {
            ...
            string resourceStringGroup = rm.GetString($"PlaceholderGroup{placeholders.Value.Item1}") ?? "?";
            ...
            DocumentPlaceholderViewConfig placeholderViewConfig = new DocumentPlaceholderViewConfig()
            {
                ...
                Placeholders = string.Join(Environment.NewLine, placeholders.Value.Item2.Select(p => $"{_documentService.PlaceholderMarker}{p}{_documentService.PlaceholderMarker}")),
